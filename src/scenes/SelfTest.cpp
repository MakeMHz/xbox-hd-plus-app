...
{
    SceneSelfTest* scene = static_cast<SceneSelfTest *>(obj->user_data);
    scene->OnObjectEvent(obj, event);
}

SceneSelfTest::SceneSelfTest() {
    ...
    lv_label_set_text(label1, "TODO SceneSelfTest");
...
void SceneSelfTest::OnObjectEvent(lv_obj_t* obj, lv_event_t event)
{
